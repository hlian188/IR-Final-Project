...
class SearchEngine:
    ...
    def search(self, search_query, search_id, corpus, index_dict):
        print("Search query:", search_query)
        results = []
        ##### your code here #####
        # results = build_demo_results(corpus, search_id)  # replace with call to search algorithm

        ranked_docs, pred_score, title_index = search_in_corpus(search_query, index_dict)

        for d_id, score in zip(ranked_docs, pred_score):
            tweet: Document = corpus[d_id]
            results.append(ResultItem(tweet.id, tweet.title, tweet.description, tweet.doc_date,
