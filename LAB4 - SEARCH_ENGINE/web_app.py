...
from myapp.analytics.analytics_data import AnalyticsData, ClickedDoc, Query
...
infile = open(path+'/inverted_index', 'rb')
...
def search_form_post():
    ...
    results = search_engine.search(search_query, search_id, corpus, index_dict)
...
def doc_details():
    ...
    doc: Document = corpus[int(clicked_doc_id)]

    return render_template('doc_details.html', page_title="Doc Details", doc=[doc])
...
def dashboard():
    ...
    # Storing the queries and the number of times that have been searched
    for q_id in analytics_data.fact_queries:
        q = Query(q_id, analytics_data.fact_queries[q_id])
        queries.append(q)
