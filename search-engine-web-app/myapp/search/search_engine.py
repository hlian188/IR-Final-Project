...
class SearchEngine:
    ...
    def search(self, search_query, search_id, result_dict):
        print("Search query:", search_query)
        results = []
        ##### your code here #####
        # results = build_demo_results(corpus, search_id)  # replace with call to search algorithm

        ranked_docs, pred_score, title_index = search_in_corpus(search_query, result_dict)

        for d_id, score in zip(ranked_docs, pred_score):
            tweet: Document = title_index[d_id]
            print(tweet)
            results.append(ResultItem(tweet['id'], tweet['full_text'][:100], tweet['full_text'], tweet['date'],
                                      "doc_details?id={}&search_id={}&param2=2".format(tweet['id'], search_id), score))
